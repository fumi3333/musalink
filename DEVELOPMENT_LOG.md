...
## 2026-05-18 — 利用規約追加条項 / JCBチェックシート記入

### 利用規約 3 条項追加（JCB 加盟申請要件）

* **対象**: [app/legal/terms/page.tsx](app/legal/terms/page.tsx)
* **理由**: JCB CtoC チェックシートの要件（①忌避商材リスト、②債権譲渡規定、③反社勢力排除条項）が利用規約に未記載だった
* **追加内容**:
  * **第5条（出品禁止物）**: 禁止商材の具体的リスト（違法薬物・武器・偽造品・児童ポルノ・個人情報等 10 項目）
  * **第6条（債権の取扱いおよび譲渡）**: Stripe Connect 経由の決済・カード売上債権の第三者譲渡に関する規定・運営が資金を保管しない旨
  * **第7条（反社会的勢力の排除）**: 表明保証・確約・違反時のアカウント停止・通報措置
  * 旧第5条（準拠法）→ **第8条** に繰り下げ
* **影響範囲**: 利用規約ページのみ。既存機能・決済フローへの影響なし

### JCB チェックシート記入済みファイル生成

* **ファイル**: `C:\Users\hrfmt\OneDrive\ドキュメント\チェックシート_Musalink_記入済み_20260518.xlsx`
* **表紙**: 申請日 2026/5/18、企業名「松田 晴史」、店舗名「Musalink」、URL 記入
* **表紙チェック項目**: 1(売買PF)・2(忌避商材)・3(純粋個人→CtoC)・4(PF運営のみ)・5(手数料なし)・6①②(会員見え方)・7(3DS)・8(1年以上 ※要協議)・9(自社反社DB無)・10(4条件規定済み)・12①②
* **CtoC シート**: AK21/25/31/37/41/47/53/55 を True に設定 → 「取扱可（加盟相談不要）」判定
* **備考欄**: 「8.信用度」1年要件は2026年5月開始のため未達、別途相談する旨を記載
* **エスクロー欄（AK47）**: 仮押さえ決済（Authorize/Capture）で実質同等として True に設定
