...
1. **【UX】LINEアプリ内ブラウザでのカメラ起動不可対策**
   * **対象**: `components/layout/InAppBrowserGuard.tsx`
   * **内容**: 以前は「未ログイン時」のみLINEブラウザの警告を出していましたが、ログイン済みのユーザーがLINEのURLを踏んで直接取引画面を開いた場合、iOSの仕様でQRカメラが起動せず待ち合わせ場所でパニックになるリスクがありました。これを防ぐため、ログイン状態に関わらずLINEブラウザの検知と警告（「Safari/Chromeを開いてください」）を出すように強化しました。
